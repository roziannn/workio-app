        ...
        <div className="lg:col-span-8 bg-white p-4 sm:p-6 rounded-3xl h-[250px] sm:h-[300px]">
          ...
          <Line
            ...
            options={{
              ...
              scales: { x: { grid: { display: false }, ticks: { color: "#555" } }, y: { beginAtZero: true, grid: { display: false }, ticks: { color: "#555" } } },
            }}
          />
        </div>
      </div>
              ...
              <li key={task.id} className="flex flex-wrap justify-between items-center px-3 sm:px-4 py-2 sm:py-3 rounded-full bg-slate-50 hover:bg-gray-100 transition">
                ...
                <span
                  className={`text-xs sm:text-sm font-semibold px-2 sm:px-3 py-1 sm:py-2 rounded-full ${
                    task.status === "Done" ? "bg-[#E8F7F4] text-green-600" : task.status === "In Progress" ? "bg-[#FFF2EB] text-orange-600" : "bg-gray-200 text-gray-500"
                  }`}
                >
                  {task.status}
                </span>
